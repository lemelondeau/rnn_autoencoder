...
class myAutoEncoder():
    def __init__(self, max_level, hidden_dim, train_filename="data_4base_5level.csv", test_filename=None):
        ...
        self._batch_num_test = int(
            np.ceil(len(self.testdata) / self._batch_size))
        self.length = self.getLength(self.data_X)
        self.reverse_y = self.reversedata(self.data_X)
        self.reverse_testy = self.reversedata(self.testdata)
        self._batch_num = int(np.ceil(len(self.data_X) / self._batch_size))

        # [batch_size, num_steps] self._batch_size
        self.x = tf.placeholder(tf.int32, [None, self._max_length])
    ...
    def getNextBacth(self, batch_size, num):
        ...
        # if e exceeds the size of data, will ignore automatically
...
if __name__ == '__main__':
   ...
   # filename="data_"+str(num_base)+"base_"+str(max_level)+"level.csv"
    train_filename = "data_4base_5level.csv"
    test_filename = "kervec_6dataset.csv"
    myAutoEncoder(max_level, hidden_dim, train_filename,
                  test_filename=None)
