...
class rnnKeras():
    ...
    def __init__(self, start, max_level, hidden_dim, filename="data_4base_5level.csv"):
        ...
        model.add(SimpleRNN(hidden_dim, input_shape=(self.data_dim, embedding.shape[
                  1]), return_sequences=True, activation='sigmoid'))
        model.add(Dense(1))
        ...
        get_hidden_output = K.function([model.layers[0].input], [
        ...
        for i in range(self.data_dim):
            ...
            output_h[s:e, :] = get_hidden_output(
                [X_all_data[s:e, :]])[0][:, i, :]

        df = pd.DataFrame(output_h[0:len(data), :])
        df.to_csv("onernn_hidden" + filename[4:len(filename) - 4] +
                  "_" + str(hidden_dim) + ".csv", float_format="%.5f")
